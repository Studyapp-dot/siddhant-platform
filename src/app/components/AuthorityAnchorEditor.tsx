...
import { createAuthorityAnchor, deleteAuthorityAnchor, searchArchiveNodes } from '@/app/actions/authority-anchors';
import { AUTHORITY_TYPE_META } from '@/app/actions/authority-constants';
import type { AuthorityType, AuthorityAnchor } from '@/app/actions/authority-anchors';
...
// PROPS
// ============================================================================
interface AuthorityAnchorEditorProps {
  nodeId: string;
  slug: string;
  revisionId: string;
...
// COMPONENT
// ============================================================================
export default function AuthorityAnchorEditor({
  nodeId, slug, revisionId, existingAnchors, textareaRef,
}: AuthorityAnchorEditorProps) {
  // ── Selection state ──
  ...
  // ── Anchors list ──
  const [anchors, setAnchors] = useState<AuthorityAnchor[]>(existingAnchors);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.set('revision_id', revisionId);
      formData.set('slug', slug);
      formData.set('anchor_text', selectedText);
      formData.set('context_before', contextBefore);
      formData.set('context_after', contextAfter);
      formData.set('paragraph_index', String(paragraphIndex));
      formData.set('authority_type', authorityType);
      formData.set('authority_title', authorityTitle.trim());
      if (authorityCitation) formData.set('authority_citation', authorityCitation);
      if (authorityUrl) formData.set('authority_url', authorityUrl);
      if (linkedNodeId) formData.set('authority_node_id', linkedNodeId);

      await createAuthorityAnchor(formData);

      // Optimistic UI update
      const newAnchor: AuthorityAnchor = {
        ...
        authority_node_slug: linkedNodeId ? null : null,
      ...
      console.error('[authority-anchor] Submit failed:', err);
    } finally {
  ...
  const handleDelete = async (anchorId: string) => {
    ...
    try {
      await deleteAuthorityAnchor(anchorId, slug);
      setAnchors(prev => prev.filter(a => a.id !== anchorId));
    } catch (err) {
      console.error('[authority-anchor] Delete failed:', err);
    } finally {
      setDeletingId(null);
    }
  };
          ...
          <div className="authority-panel">
            ...
            <div className="authority-panel-body">
              ...
              <button
                ...
                disabled={!authorityTitle.trim() || isSubmitting}
                onClick={handleSubmit}
              >
                {isSubmitting ? 'Attaching...' : 'Attach Authority'}
              </button>
            </div>
          </div>
        </div>
      ...
      {anchors.length > 0 && (
        <div className="authority-existing-section">
          <div className="authority-existing-header">
            ...
            <span className="authority-existing-count">{anchors.length}</span>
          </div>
